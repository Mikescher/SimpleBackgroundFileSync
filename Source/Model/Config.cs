	...
	public static class Config
	{
		...
		public static void Reload()
		{
			...
			foreach (var entry in xdoc.Root?.Elements("file") ?? new List<XElement>())
			{
				...
				var ufw = entry.Attribute("filewatcher")?.Value ?? "false";

				Entries.Add(new ConfigEntry(src, dst, int.Parse(ivl), bool.Parse(ufw), ParseComparisonMode(cm), ParseErrorMode(em1)));
			}
		}
